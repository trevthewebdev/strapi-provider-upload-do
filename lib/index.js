...
class FileLocationConverter {
  static getKey(config, options, file) {
    const directory = options.directory || config.directory;
    return `${directory ? `${directory}/` : ""}${file.hash}${file.ext}`;
  }

  static getUrl(config, data) {
    if (config.cdn)
      return data.Location.replace(
        `${config.space}.${config.endpoint}`,
        config.cdn
      );
    ...
    return {
      upload: (file, options) =>
        new Promise((resolve, reject) => {
          ...
          S3.upload(
            {
              Key: FileLocationConverter.getKey(config, options, file),
